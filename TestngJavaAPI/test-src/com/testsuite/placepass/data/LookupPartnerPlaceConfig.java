...
//	Declaration of respective API Parts instances
	...
	//	Verify links returned in shelves
		List<String> links = data.verifyPartnerPlaceConfigWebIdGuideCountParam(webid, title, location, carLocationCode,env );
		HashMap<String, String> response = null;
		for(int i=0; i<links.size();i++){
			response = data.requestShelflink(links.get(i),env);
